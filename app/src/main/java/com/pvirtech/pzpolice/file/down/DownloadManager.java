        ...
        try {
            ...
            if (mDownloadInfoList != null && mDownloadInfoList.size() > 0) {
                for (DownloadInfo downloadInfo : mDownloadInfoList) {
                    if (downloadInfo.getState() == FileState.NONE || downloadInfo.getState() == FileState.WAITING || downloadInfo
                            .getState() == FileState.ERROR) {
                        downloadInfo.setState(FileState.DOING);
                        task = downloadInfo;
                        break;
                    }
                }
